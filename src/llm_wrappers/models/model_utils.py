...
# Assuming this is inside your Python code within src/llm_wrappers/
current_dir = os.path.dirname(os.path.abspath(__file__))  # Directory where the script is
project_root = os.path.join(current_dir, "../../..")       # Go up three levels to reach the root

data_folder = os.path.join(project_root, "data")

from llm_wrappers.utils import get_api_key, printv
...
def get_model_details(model_name, fpath=f'{data_folder}/llm_model_details.yaml'):
    print(os.listdir())
    try:
...
def get_api_settings(api_provider, fpath=f'{data_folder}/api_settings/apis.yaml'):
    try:
        with open(fpath) as f:
            details = yaml.safe_load(f)
    except Exception as e:
